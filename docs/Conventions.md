...
## String quotes.
Single quotes should be used for strings that represent constant/fixed values. Such as in-place enums, Love2D's DrawMode, or comparing the return value of Lua's `type` function.
Double quotes should be used for everything else, that fits on a single line. Such as file paths, text to be displayed, object ID's that are loaded from datapacks, etc...
Brackets should be used for larger blocks of text, which spans many lines.

Example of all of the above:
```lua
Game = require "core.Game"
Game:setGameName("My Purrfect Game")
data = {ID = "stone_sword"}
if type(DataRegistry) == 'table' then
    DataRegistry:applyStats(data)
end
g2d.setDefaultFilter('nearest', 'nearest')
print("Setup done.")
print([[
Game controls:
Arrow Keys    ;    Move
...
6. Extremely short blocks (plain getters, quick loops, etc...) can be written on one line.
7. When hard-wrapping a line;
...
8. Always use parenthesis when calling a function, even if it only takes a single string or table. The only exception is `require`.
...
## Engine Conventions
Those are only relevant if you are contributing to the source of CatPaw. Follow them in addition to everything else above.
1. Tabs for indentation.
2. If you are wrapping a line which contains a list of `and`/`or`s, wrapping should come *after* an `and`/`or`.
*See the example in `#Other/6`.*
3. If a function/method takes an argument that is a table used to hold options/configurations (or to act as *named arguments*) that argument should be named `opt`.
